...
const correoUsuario = "usuario@ejemplo.com"; // Correo del usuario, esto puede obtenerse de una variable global o del estado de la aplicación
...
const actualizarFoto = async () => {
  ...
  const correo = usuario ? usuario.correo : null;
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      const updatedUser = { ...usuario, fileInput: nuevaRuta };  // Usamos "fileInput" en lugar de "imagen"

      // Asegúrate de guardar correctamente el objeto actualizado en localStorage
      ...
      usuario.fileInput = nuevaRuta;  // Asegurando que el estado local también se actualice
    }
  } catch (error) {
...
const actualizarNombre = async () => {
  try {
    ...
    if (response.status === 200) {
      ...
      usuario.nombreUsuario = nuevoNombre.value;
...
const actualizarCiudad = async () => {
  try {
    ...
    if (response.status === 200) {
      ...
      usuario.ciudad = nuevaCiudad.value;
...
const actualizarDescripcion = async () => {
  try {
    ...
    if (response.status === 200) {
      ...
      usuario.descripcion = nuevaDescripcion.value;
