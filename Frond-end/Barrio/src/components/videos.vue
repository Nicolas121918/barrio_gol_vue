      ...
      <transition-group name="slide" tag="div" class="video-list">
        ...
        >
          ...
          <div class="video-info">
            <img :src="video.uploaderProfilePic" alt="Profile" class="profile-pic" />
            <div class="details">
...
export default {
  ...
  computed: {
    currentVideo() {
      return this.videosurls[this.currentIndex];
    },
  },
  mounted() {
    this.getvideosurl();
  ...
  methods: {
    ...
    nextVideo() {
      if (this.currentIndex < this.videosurls.length - 1) {
        this.currentIndex++;
        this.playCurrentVideo();  // Reproducir el video actual
      }
    },
    previousVideo() {
    ...
    playCurrentVideo() {
      ...
      if (video) {
        video.play();  // Reproducir el video correspondiente
      }
    ...
    likeVideo(index) {
      this.videosurls[index].likes++;
      this.likedIndex = index;  // Activar animación para este botón
      setTimeout(() => {
        this.likedIndex = null; // Resetear animación
      }, 500);
    },
    addComment(index) {
      const comment = this.videosurls[index].newComment;
      if (comment.trim()) {
        this.videosurls[index].comments.push(comment);
        this.videosurls[index].newComment = '';  // Limpiar el comentario después de agregarlo
      }
    },
    getUrlvideo(path) {
