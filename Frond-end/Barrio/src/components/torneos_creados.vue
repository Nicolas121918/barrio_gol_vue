  ...
  <header>
    ...
    <div class="tournaments-container">
      ...
      <div class="search-container">
        ...
        <router-link class="padre" to="/torneos">
            <button @click="goBack" class="btn-donate">Volver</button>
      </router-link>
        
      </div>
      <!-- Apartado de Torneos y Partidos Creados -->
      <div class="section created-section">
        <div class="children">
        <h2>Torneos y Partidos Creados</h2>
    </div>
        <div class="card-container">
          <!-- Mostrar mensaje si no hay resultados -->
          <p v-if="filteredCreatedItems.length === 0" class="no-results-message">
         ¡Por ahora no hay resultados para tu búsqueda.!
          </p>
  
          <!-- Mostrar las tarjetas si hay resultados -->
          <transition-group name="fade" tag="div" class="card-list">
            <div 
              v-for="item in filteredCreatedItems" 
              :key="item.id" 
              class="card"
            >
              ...
              <div class="butons">
              <button v-if="item.type === 'torneo'" @click="editItem(item)" class="card-button">Editar</button>
              <button v-else @click="editItem(item)" class="card-button">Editar</button>
            </div>
            </div>
          </transition-group>
        </div>
